...
export default function Controls() {
  ...
  function handleAntiAfkInterval(accountId: string, intervalMs: number) {
    socket.emit('bot:anti_afk', { account_id: accountId, enabled: true, interval_ms: intervalMs });
  }

  // Mouse look: send yaw/pitch deltas to all move targets
  function handleMouseLook(e: MouseEvent) {
    const targets = getMoveTargets();
    if (targets.length === 0) return;
    const yawDelta = -e.movementX * SENSITIVITY;
    const pitchDelta = -e.movementY * SENSITIVITY;
    for (const id of targets) {
      socket.emit('bot:look', { account_id: id, yaw_delta: yawDelta, pitch_delta: pitchDelta });
    }
  }

  // Enter/exit pointer lock for mouse look
  ...
  useEffect(() => {
    function onLockChange() {
      setMouseLook(document.pointerLockElement !== null);
    }
  ...
  // Keyboard controls for movement (with hold detection for mouse look activation)
  useEffect(() => {
        ...
        <motion.section variants={sectionVariants} transition={{ type: 'spring', stiffness: 200, damping: 20 }}>
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
            ...
            <div className="flex flex-1 flex-col items-center justify-center gap-3">
              <div ref={dpadRef} className="grid grid-cols-3 grid-rows-3 gap-1" style={{ width: 132, height: 132 }}>
